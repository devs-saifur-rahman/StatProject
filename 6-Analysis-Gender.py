...
label_encoders = {}
for column in ['Gender', 'Made or received a digital payment']:
    le = LabelEncoder()
...
# Sigmoid Curve for Gender
Gender_values = np.sort(data[independent_var].unique())
X_test = pd.DataFrame({independent_var: Gender_values})
X_test = sm.add_constant(X_test)
y_test_pred_prob = logit_model.predict(X_test)

plt.figure(figsize=(10, 7))
plt.plot(Gender_values, y_test_pred_prob, color='red', linewidth=3)
plt.scatter(data[independent_var], y, color='black', zorder=20, alpha=0.5)
plt.xlabel('Gender')
plt.ylabel('Probability of Digital Payment')
plt.title('Logistic Regression Sigmoid Curve for Gender')
plt.show()
