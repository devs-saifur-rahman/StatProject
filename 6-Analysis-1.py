...
# Sigmoid Curve for Region
region_values = np.sort(data[independent_var].unique())
X_test = pd.DataFrame({independent_var: region_values})
X_test = sm.add_constant(X_test)
y_test_pred_prob = logit_model.predict(X_test)

plt.figure(figsize=(10, 7))
plt.plot(region_values, y_test_pred_prob, color='red', linewidth=3)
plt.scatter(data[independent_var], y, color='black', zorder=20, alpha=0.5)
plt.xlabel('Region')
plt.ylabel('Probability of Digital Payment')
plt.title('Logistic Regression Sigmoid Curve for Region')
plt.show()
